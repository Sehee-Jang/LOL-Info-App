...
export async function getChampionDetail(id: string): Promise<ChampionDetail> {
  const latestVersion = await getLatestVersion();
  const championsUrl = `${BASE_URL}/${latestVersion}/data/ko_KR/champion/${id}.json`;
...
export async function getItems(): Promise<{ [key: string]: Item }> {
  const latestVersion = await getLatestVersion();
  const championsUrl = `${BASE_URL}/${latestVersion}/data/ko_KR/item.json`;
